...
function About() {
  return <>
    ...
    <section><div className="mx-auto max-w-[1240px] px-6 py-24 md:px-10"><div className="flex flex-wrap items-end justify-between gap-6"><div><p className="font-mono text-[11px] uppercase tracking-[0.22em] text-primary">Experience</p><h2 className="mt-4 text-4xl font-semibold md:text-6xl">The work behind the work.</h2></div><Link to="/contact" className="inline-flex items-center gap-2 text-sm text-primary">Work with me <ArrowUpRight size={16} /></Link></div><div className="mt-14 divide-y divide-border border-y border-border">{[["GenRev", "Web Designer", "2025—2026"], ["Mango Entertainment", "Web Designer", "2024—2025"], ["Tax & Compliance", "Web Designer & Writer", "2023—2024"], ["Lorchain", "Technical Writer", "2024—2025"]].map(([company, role, date]) => <div key={company} className="grid gap-2 py-6 md:grid-cols-12 md:items-center"><p className="font-tech text-xl font-semibold md:col-span-5">{company}</p><p className="text-sm text-muted-foreground md:col-span-4">{role}</p><p className="font-mono text-xs text-primary md:col-span-3 md:text-right">{date}</p></div>)}</div></div></section>
  </>;
}
